  ...
  \begin{enumerate}
    ...
    \includegraphics[height=10cm, width=12cm]{One.JPG}
      ...
      \textcolor{hwColor}{
        ...
        \\
        $
          p^m_n=\bigints r^n_s \rho_Q(\overrightarrow{r_s}) Y^{m*}_n(\theta, \phi) d^3 \overrightarrow{r_s} \\
          \\
          \\
          =\bigints r^n_s \rho_Q(\overrightarrow{r_s}) Y^{m*}_n(\theta, \phi) r^2_s sin(\theta) dr_s d\theta d\phi \\
          \\
          \\
          =\bigints r^n_s \rho_Q(\overrightarrow{r_s}) Y^{m*}_n(\theta, \phi) r^2_s sin(\theta) d^3V \\
          \\
          \\
          =\bigints r^n_s \left[-\dfrac{Q}{r^2}\delta(r) \dfrac{Q}{r^2} \left(\delta(u-1)+\delta(u)\right) \delta(r-d)\left(\delta(\phi)+\delta(\phi_1)\right)\right] Y^{m*}_n(\theta, \phi) r^2_s sin(\theta) d^3V \\
          \\
          \\
          =\bigints Y^{m*}_n r^n_s \left[Q  \left(\delta(u-1)+\delta(u)\right) \left(\delta(r-d)\right) \delta(\phi_1) \right] sin(\theta) d^3V
          -\bigints Q Y^{m*}_n r^n_s \delta(r) \delta(\phi) \delta(\theta) sin(\theta) d^3V \\
          \\
          \\
          =\bigints Y^{m*}_n r^n_s \left[Q  \left(\delta(u-1)+\delta(u)\right) \left(\delta(r-d)\right) \delta(\phi_1) \right] du ~ dr ~ d\Phi
          -\bigints Q Y^{m*} r^n_s \delta(r) \delta(\phi) \delta(\theta) du ~ dr ~ d\Phi \\
          \\
          \\
          =Q \left[
            d^n \left(\bigints Y^{m*}_n \delta(u-1) \delta(\phi_1) ~ du ~ d\phi+\bigints Y^{m*}_n \delta(u) \delta(\phi_1) ~ du ~ d\phi  \right)
            -\bigints Y^{m*}_n r^n_s \delta(r) \delta(\phi) \delta(\theta) du ~ dr ~ d\phi
          \right]
          \\
          \\
          \\
          \therefore ~~~~ p^m_n=Qd^n(a)-Q(b) ~~~ \surd \\
          \\
          \\
          \\
          \\
          Y^{m*}_n(\theta, \phi)=(-1)^m \left[\dfrac{(2n+1)(n-m)!}{4\pi (n+m)!}\right] p^m_n(u) e^{-im \phi}=(-1) p^m_n(u) \alpha e^{-im \phi} \\
          \\
          \\
          \\
          \\
          ...
          \\
          \\
          \therefore ~~~~ b=\delta_{m,0} \left[(-1)^m \alpha p^m_n(0) \delta_{n,0}\right]  ~~~ \surd
        $ \\
        \\
        \\
        $
          p^m_n=Qd^n (-1)^m \alpha \delta_{m,0} \left[p^m_n(1)+p^m_n(0)\right]-Q \delta_{m,0} \left[(-1)^m \alpha p^m_n(0) \delta_{n,0}\right] \\
          \\
          \\
          \\
          \therefore ~~~~ p^m_n=Q \alpha (-1)^m \delta_{m,0} \left[d^n \left(p^m_n(1)+p^m_n(0)\right)-\delta_{n,0} p^m_n(0) \right] ~~~ \surd 
        $ \\
        \\
        \\
        \textbf{The monopole moment:} \\
        \\
        $
          p^0_0=Q\left[ \dfrac{(2(0)+1)(0-0)!}{4 \pi (0+0)!}\right]^{\dfrac{1}{2}} (-1)^0 \left[d^0\left(p^0_0(1)+p^0_0(0)\right) -\delta_{0,0} p^0_0(0)\right] \\
          \\
          \\
          \\
          \therefore ~~~ p^0_0=\sqrt{\dfrac{1}{4 \pi}} ~~~ \surd
        $ \\
        \\
        \\
        \rule{15cm}{1pt}
        \\
        \\
        \textbf{The dipole moment } \\
        \\
        $
          p^0_1=Q\left[ \dfrac{(2(1)+1)(1-0)!}{4 \pi (0+1)!}\right]^{\dfrac{1}{2}} (-1)^0 \left[d^1\left(p^0_1(1)+p^0_1(0)\right) -\delta_{0,1} p^0_1(0)\right] \\
          \\
          \\
          \\
          \therefore ~~~ p^0_1=Q ~ d\sqrt{\dfrac{3}{4 \pi}} ~~~ \surd
        $
      }
